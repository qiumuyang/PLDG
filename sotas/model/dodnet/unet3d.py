...
class Conv3d(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        weight_mean = weight.mean(dim=1, keepdim=True).mean(
            dim=2, keepdim=True).mean(dim=3, keepdim=True).mean(dim=4,
                                                                keepdim=True)
...
class unet3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_subtasks=1,
                 weight_std=False):
        # original: organ + tumor => num_subtasks=2 and num_classes not used
        # current: organ only => num_subtasks=1 and num_classes used for task encoding
        self.inplanes = 128
        self.weight_std = weight_std
        ...
        self.conv1 = conv3x3x3(num_channels,
                               32,
                               stride=[1, 1, 1],
                               weight_std=self.weight_std)

        self.layer0 = self._make_layer(NoBottleneck,
                                       32,
                                       32,
                                       layers[0],
                                       stride=(1, 1, 1))
        self.layer1 = self._make_layer(NoBottleneck,
                                       32,
                                       64,
                                       layers[1],
                                       stride=(2, 2, 2))
        self.layer2 = self._make_layer(NoBottleneck,
                                       64,
                                       128,
                                       layers[2],
                                       stride=(2, 2, 2))
        self.layer3 = self._make_layer(NoBottleneck,
                                       128,
                                       256,
                                       layers[3],
                                       stride=(2, 2, 2))
        self.layer4 = self._make_layer(NoBottleneck,
                                       256,
                                       256,
                                       layers[4],
                                       stride=(2, 2, 2))
        ...
        self.fusionConv = nn.Sequential(
            nn.GroupNorm(16, 256), nn.ReLU(inplace=in_place),
            conv3x3x3(256,
                      256,
                      kernel_size=(1, 1, 1),
                      padding=(0, 0, 0),
        ...
        self.x8_resb = self._make_layer(NoBottleneck,
                                        256,
                                        128,
                                        1,
                                        stride=(1, 1, 1))
        self.x4_resb = self._make_layer(NoBottleneck,
                                        128,
                                        64,
                                        1,
                                        stride=(1, 1, 1))
        self.x2_resb = self._make_layer(NoBottleneck,
                                        64,
                                        32,
                                        1,
                                        stride=(1, 1, 1))
        self.x1_resb = self._make_layer(NoBottleneck,
                                        32,
                                        32,
                                        1,
                                        stride=(1, 1, 1))

        self.precls_conv = nn.Sequential(nn.GroupNorm(16, 32),
                                         nn.ReLU(inplace=in_place),
                                         nn.Conv3d(32, 8, kernel_size=1))

        self.GAP = nn.Sequential(nn.GroupNorm(16, 256),
                                 nn.ReLU(inplace=in_place),
                                 torch.nn.AdaptiveAvgPool3d((1, 1, 1)))
        self.controller = nn.Conv3d(256 + self.num_classes,
                                    144 + self.num_subtasks * 9,
                                    kernel_size=1,
                                    stride=1,
                                    padding=0)
    ...
    def _make_layer(self,
                    ...
                    multi_grid=1):
        downsample = None
        if stride[0] != 1 or stride[1] != 1 or stride[
                2] != 1 or inplanes != planes:
    ...
    def encoding_task(self, task_id):
        N = task_id.shape[0]
        task_encoding = torch.zeros(size=(N, self.num_classes),
    ...
    def parse_dynamic_params(self, params, channels, weight_nums, bias_nums):
        ...
        for l in range(num_layers):
            if l < num_layers - 1:
                ...
                weight_splits[l] = weight_splits[l].reshape(
                    num_insts * self.num_subtasks, -1, 1, 1, 1)
    ...
    def forward(self, input, task_id):
        ...
        weight_nums.append(8 * 8)
        weight_nums.append(8 * self.num_subtasks)
        bias_nums.append(8)
        bias_nums.append(8)
        bias_nums.append(self.num_subtasks)
        weights, biases = self.parse_dynamic_params(params, 8, weight_nums,
                                                    bias_nums)
        logits = self.heads_forward(head_inputs, weights, biases, N)
        logits = logits.reshape(-1, self.num_subtasks, D, H, W)

        return logits


def UNet3D(*, num_channels=1, num_classes=1, weight_std=False):
    model = unet3D([1, 2, 2, 2, 2],
                   num_channels,
                   num_classes,
