...
class Conv3dBlock(nn.Module):
    ...
    def __init__(self, cin, cout=None, cmid=None, down_factor=1):
        ...
        if down_factor != 1:
            self.downsample = nn.Upsample(scale_factor=down_factor,
                                          mode='trilinear',
                                          align_corners=True)
            ...
            OrderedDict([
                ('block1',
                 nn.Sequential(
                     OrderedDict([
                         ('unit1',
                          Conv3dBlock(cin=width, cout=width * 2, cmid=width))
                     ] + [(f'unit{i:d}',
                           Conv3dBlock(
                               cin=width * 2, cout=width * 2, cmid=width))
                          for i in range(2, block_units[0])] +
                                 [('unit' + str(block_units[0]),
                                   Conv3dBlock(cin=width * 2,
                                               cout=width * 4,
                                               cmid=width * 2,
                                               down_factor=0.5))], ))),
                ('block2',
                 nn.Sequential(
                     OrderedDict([
                         ('unit1',
                          Conv3dBlock(
                              cin=width * 4, cout=width * 4, cmid=width * 2))
                     ] + [(f'unit{i:d}',
                           Conv3dBlock(
                               cin=width * 4, cout=width * 4, cmid=width * 2))
                          for i in range(2, block_units[1])] +
                                 [('unit' + str(block_units[1]),
                                   Conv3dBlock(cin=width * 4,
                                               cout=width * 8,
                                               cmid=width * 2,
                                               down_factor=0.5))], ))),
                ('block3',
                 nn.Sequential(
                     OrderedDict([
                         ('unit1',
                          Conv3dBlock(
                              cin=width * 8, cout=width * 8, cmid=width * 4))
                     ] + [(f'unit{i:d}',
                           Conv3dBlock(
                               cin=width * 8, cout=width * 8, cmid=width * 4))
                          for i in range(2, block_units[2] + 1)] +
                                 [('unit' + str(block_units[2]),
                                   Conv3dBlock(cin=width * 8,
                                               cout=width * 16,
                                               cmid=width * 4,
                                               down_factor=0.5))], ))),
            ]))
