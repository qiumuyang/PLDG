...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        max_along_axis = torch.max(attention_scores, dim=-1,
                                   keepdim=True).values
        exp_logits = torch.exp(attention_scores - max_along_axis)
        attention_probs = exp_logits / torch.sum(
            exp_logits, dim=-1, keepdim=True)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        n_patches = (img_size[0] // patch_size_real[0]) * (
            img_size[1] // patch_size_real[1]) * (img_size[2] //
                                                  patch_size_real[2])

        self.hybrid_model = FeatureExtractor(
        ...
        if config.hidden_size != 512:
            self.modality_embedding = Linear(in_features=512,
    ...
    def forward(self, x, modality=None):
        ...
        x = self.patch_embeddings(
            x
        )  # (B, hidden, n_patches^(1/3), n_patches^(1/3), n_patches^(1/3))
...
class DecoderCup(nn.Module):
    ...
    def __init__(self, config):
        ...
        out_channels = self.config.skip_channels[1:] + [
            self.config.skip_channels[-1]
        ]

        use_batchnorms = [config.decoder_use_bn] * len(in_channels)

        blocks = [
            DecoderBlock(in_ch, out_ch, sk_ch, use_bn)
            for in_ch, out_ch, sk_ch, use_bn in zip(
                in_channels, out_channels, skip_channels, use_batchnorms)
        ]
        ...
        self.conv2 = nn.Conv3d(out_channels[-1],
                               out_channels[-1],
                               kernel_size=3,
                               padding=1)
    ...
    def forward(self, hidden_states, features=None, level1=None):
        ...
        )  # reshape from (B, n_patch, hidden) to (B, d, h, w, hidden)
        d, h, w = int(np.cbrt(n_patch)), int(np.cbrt(n_patch)), int(
            np.cbrt(n_patch))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, gt=None):
        ...
        x, attn_weights, features, full = self.transformer(
            x)  # (B, n_patch, hidden)
